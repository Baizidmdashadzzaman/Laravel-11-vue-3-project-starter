...
const { state_category, } = usemodelData();

const { validationErrors,urlcall,loadingBtn,storeData,addingstatus } = useDataCall();

const emit = defineEmits(['changeActive'])
let pagename = ref('Category');
let url_data = ref('category');
...
const singledatapage = reactive({ ...state_category });

const saveData = async () => {
   await storeData(singledatapage);
   if(addingstatus.value){
      for (const [key, value] of Object.entries(state_category)) {
...
const onImageChange2 = (e) => {
   singledatapage.banner = e.target.files[0];
};
