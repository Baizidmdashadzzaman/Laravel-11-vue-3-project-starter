...
const { state_delivarycharge, } = usemodelData();

const { validationErrors,urlcall,loadingBtn,storeData,addingstatus } = useDataCall();

const emit = defineEmits(['changeActive'])
let pagename = ref('Delivary charge');
let url_data = ref('delivarycharge');
...
const singledatapage = reactive({ ...state_delivarycharge });

const saveData = async () => {
   await storeData(singledatapage);
   if(addingstatus.value){
      for (const [key, value] of Object.entries(state_delivarycharge)) {
