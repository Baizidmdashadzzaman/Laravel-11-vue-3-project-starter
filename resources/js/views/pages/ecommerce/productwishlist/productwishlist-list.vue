...
const { state_product,state_productwishlist } = usemodelData();
...
let pagename = ref('Product wishlist');
let url_data = ref('productwishlist');


const singledatapage = reactive({ ...state_product });
const singledatapagecust = reactive({ ...state_productwishlist });

let alldata_customer = ref({});
const getCustomerInfo = async () => {
    const datapro = await getResultAll('/customer/edit/'+route.params.id)
    alldata_customer.value = datapro.alldata;
    loading.value = false;
};

onMounted(() => {
  emit('changeActive', 'customer')
  urlcall.value = url_data.value;
  getCustomerInfo();
  get_allproduct();
  singledatapagecust.customer_id = route.params.id;
});
...
const getDataAllCustom = async () => {
    const data = await getResultAll('/productwishlist/list-all-customer-wise/'+route.params.id)
...
const editDataCustom = async (id) => {
   ...
   .then((response) => {
       ...
       modal_title.value = 'Update product wishlist';
       modal_show.value = true;
   });
}
...
const storeData = async (formdata) => {
        ...
        let urlForm = '';
        if(formdata.id === ''){ urlForm = '/productwishlist/store'; }
        else{ urlForm = '/productwishlist/update/'+formdata.id; }
...
<div class="">
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">
        <RouterLink :to="{name:'admin.dashboard'}">Dashboard</RouterLink> / </span>{{ pagename }} / List / <span class="btn btn-outline-success btn-sm waves-effect waves-light">{{ alldata_customer.name }}-{{ alldata_customer.phone }}</span></h4>
   <div class="card mb-4">
      <div class="row ms-2 me-3 b">
         <div class="col-12 col-md-6 d-flex align-items-center justify-content-center justify-content-md-start gap-2 border-bottom">
            ...
            <div class="dt-action-buttons text-xl-end text-lg-start text-md-end text-start mt-md-0 mt-3">
               <div class="dt-buttons btn-group flex-wrap">
                  <h5 class="card-header" style="padding-left: 0px;">Product wishlist information</h5>
               </div>
            </div>
         </div>
         <div class="col-12 col-md-6 d-flex align-items-center justify-content-end flex-column flex-md-row pe-3 gap-md-2 border-bottom">
            <div id="DataTables_Table_0_filter" class="dataTables_filter">
               ...
               &nbsp;&nbsp;
               <button @click="addDataModal" class="btn btn-secondary btn-success"  type="button"><span><i class="ti ti-square-rounded-plus me-md-2"></i><span class="d-md-inline-block d-none">Add wishlist</span></span>
               </button>
            </div>
        ...
        <div class="modal-dialog">
          <form @submit.prevent="storeData(singledatapagecust)" enctype="multipart/form-data" class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="">
                <div class="col mb-3">
                  <label for="nameBackdrop" class="form-label">Product</label>
                  <select v-model="singledatapagecust.product_id" class="form-select">
                     <option value='' disabled>--please select--</option>
                     <option v-for="(value, key) in allproduct" :value="value.id" :key="key">{{ value.name }}-{{ value.code }}</option>
                  </select>
                </div>
                <div class="col mb-3">
                     ...
                     <table class="table table-hover ">
                        ...
                        <tbody class="table-border-bottom-0">
                           <tr v-for="singledata in alldata_stock" :key="singledata.id">
                              ...
                              <td><b>{{ singledata.product?.name }}</b></td>
                              <td><b>{{ singledata.product?.original_price }}</b></td>
                              <td><b>{{ singledata.product?.sell_price }}</b></td>
