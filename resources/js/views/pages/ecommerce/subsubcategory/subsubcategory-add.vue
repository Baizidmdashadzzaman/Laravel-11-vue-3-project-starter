...
const { state_subsubcategory, } = usemodelData();

const { validationErrors,urlcall,loadingBtn,storeData,addingstatus } = useDataCall();

const { allcategory,get_allcategory,allsubcategory,get_allsubcategory } = useReuseableData();


const emit = defineEmits(['changeActive'])
let pagename = ref('Subsubcategory');
let url_data = ref('subsubcategory');
...
const singledatapage = reactive({ ...state_subsubcategory });

const saveData = async () => {
   await storeData(singledatapage);
   if(addingstatus.value){
      for (const [key, value] of Object.entries(state_subsubcategory)) {
      ...
      <form @submit.prevent="saveData()" class="card-body" enctype="multipart/form-data">
         ...
         <div class="row g-3">
            ...
            <div class="col-md-6">
              ...
              <div class="input-group input-group-merge">
                <select v-model="singledatapage.subcategory_id" class="form-select">
                     <option value='' disabled>--please select--</option>
