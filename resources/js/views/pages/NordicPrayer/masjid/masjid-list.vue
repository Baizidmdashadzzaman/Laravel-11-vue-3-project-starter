                     ...
                     <table class="table table-hover ">
                        ...
                        <tbody class="table-border-bottom-0">
                           <tr v-for="singledata in alldata.data" :key="singledata.id">
                              ...
                              <td><b>{{ singledata.country?.name }}</b></td>
                              <td><b>{{ singledata.citynp?.name }}</b></td>
                              <td><b>{{ singledata.name }}</b></td>
                              <td><span class="badge bg-label-success me-1" >{{ singledata.status }}</span></td>
                              <td>
                                 <div class="btn-group" role="group" aria-label="Basic example">
                                    <RouterLink :to="{ name: 'admin.salahtime.list', params: { id: singledata.id } }" class="btn btn-outline-success btn-sm waves-effect waves-light">
                                       <i class="ti ti-calendar-time me-md-2"></i>
                                    </RouterLink>
                                    <RouterLink :to="{ name: 'admin.displaymessage.list', params: { id: singledata.id } }" class="btn btn-outline-success btn-sm waves-effect waves-light">
                                       <i class="ti ti-message me-md-2"></i>
                                    </RouterLink>
